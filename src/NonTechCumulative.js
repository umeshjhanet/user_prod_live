...
const NonTechCumulative = ({userData}) => {
  ...
  const [showConfirmationLocation, setShowConfirmationLocation] =
    useState(false);
  ...
  const fetchDetailedLocationWiseReportCsvFile = (
    locationName,
    startDate,
    endDate
  ) => {
    ...
    if (locationName && formattedStartDate && formattedEndDate) {
      apiUrl += `?locationName=${locationName}&startDate=${formatDate(
        formattedStartDate
      )}&endDate=${formatDate(formattedEndDate)}`;
    } else if (locationName) {
      apiUrl += `?locationName=${locationName}`;
    } else if (formattedStartDate && formattedEndDate) {
      apiUrl += `?startDate=${formatDate(
        formattedStartDate
      )}&endDate=${formatDate(formattedEndDate)}`;
    }
  ...
  const fetchUserWiseReportCsvFile = (
    username,
    locationName,
    startDate,
    endDate
    ...
    if (username && locationName) {
      ...
      const locationQueryString = Array.isArray(locationName)
        ? locationName.join(",")
        : locationName;
  ...
  useEffect(() => {
    ...
    //   setIsLoading(true);
    //   axios
    //     .get(`${API_URL}/summaryreportnontech`)
    //     .then((response) => {
    ...
    //   setIsLoading(true);
    //   axios
    //     .get(`${API_URL}/detailedreportcummulativenontech`)
    //     .then((response) => {
  ...
  console.log("summaryReportLocationWise", summaryReport);
  const Loader = () => (
